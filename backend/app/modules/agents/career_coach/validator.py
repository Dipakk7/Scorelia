# app/modules/agents/career_coach/validator.py
...
class CareerCoachAgentValidator:
    """Validator class for Career Coach Agent inputs, DB state, and AI outputs."""
    ...
    @staticmethod
    def validate_roadmap_exists_and_owned(db: Session, roadmap_id: uuid.UUID, user_id: uuid.UUID) -> CareerRoadmap:
        """Validates that a career roadmap exists and belongs to the given user."""
        roadmap = db.query(CareerRoadmap).filter(CareerRoadmap.id == roadmap_id).first()
        if not roadmap:
            raise AgentValidationError(f"Career roadmap with ID '{roadmap_id}' not found.")

        u_id = uuid.UUID(str(user_id)) if isinstance(user_id, str) else user_id
        if roadmap.user_id != u_id:
            raise AgentValidationError(f"Access denied: Career roadmap '{roadmap_id}' does not belong to user '{user_id}'.")
