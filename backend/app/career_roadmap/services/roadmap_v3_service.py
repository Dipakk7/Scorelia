...
class RoadmapV3Service:
    ...
    @staticmethod
    def get_skills_gap_data() -> Dict[str, Any]:
        return {
            "skillsOverview": {
                "overallReadiness": 78,
                "gapScore": 22,
    ...
    @staticmethod
    def get_milestones_data() -> Dict[str, Any]:
        return {
            "overview": {
    ...
    @staticmethod
    def get_assistant_data() -> Dict[str, Any]:
        return {
            "messages": [
                {
                    "id": "msg-1",
    ...
    @staticmethod
    def process_assistant_message(request: AssistantMessageRequest) -> Dict[str, Any]:
        now_str = datetime.datetime.now().strftime("%I:%M %p")
        ai_reply = ChatMessageSchema(
            id=f"ai-{int(datetime.datetime.now().timestamp())}",
            sender="assistant",
            text=f'Great focus! Regarding "{request.message}", I recommend prioritizing your Phase 2 Model Evaluation modules and tackling 2 coding challenges on Scikit-Learn pipelines today.',
            timestamp=now_str,
            bulletPoints=[
        ...
        base_data = RoadmapV3Service.get_assistant_data()
        user_msg = ChatMessageSchema(
            id=f"user-{int(datetime.datetime.now().timestamp())}",
            sender="user",
            text=request.message,
            timestamp=now_str,
        )
        base_data["messages"].extend([user_msg.model_dump(), ai_reply.model_dump()])
        return base_data
