...
export function KeywordCloud({ keywords }: KeywordCloudProps) {
  if (!keywords || keywords.length === 0) {
    return (
      <div className="p-8 text-center text-xs text-slate-400 dark:text-slate-500 bg-slate-50/50 dark:bg-slate-900/10 rounded-xl border border-dashed border-border">
  ...
  const getTagStyle = (type: 'matched' | 'missing' | 'suggested') => {
    switch (type) {
      case 'matched':
        return 'bg-success/10 text-success dark:bg-success/20 border-success/20 dark:border-success/30'
      case 'missing':
  ...
  return (
    <div className="p-6 bg-card/70 backdrop-blur-md rounded-2xl border border-border shadow-sm hover:border-slate-350 dark:hover:border-slate-750 transition-all duration-300 text-center font-sans">
      ...
      <div className="flex justify-center flex-wrap gap-4 mt-6 border-t border-border/60 pt-4 text-[10px] font-bold uppercase tracking-wider">
