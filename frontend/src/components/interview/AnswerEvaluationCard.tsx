...
export default function AnswerEvaluationCard({ evaluation, questionText, answerText }: AnswerEvaluationCardProps) {
  ...
  return (
    <Card className="border border-slate-205 dark:border-slate-855 bg-white/70 dark:bg-slate-900/40 backdrop-blur-md rounded-2xl shadow-sm hover:border-slate-350 dark:hover:border-slate-750 transition-all duration-300 overflow-hidden text-left space-y-6 font-sans text-xs">
      <CardContent className="p-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-4 border-b border-slate-100 dark:border-slate-800/60">
          ...
          <div className="flex items-center gap-3 bg-brand-500/5 dark:bg-brand-500/10 p-2.5 rounded-xl border border-brand-500/20 font-sans">
            ...
            <div className="text-[9px] font-sans">
              <span className="font-bold text-slate-900 dark:text-white block uppercase tracking-wider">Overall Grade</span>
            ...
            {categories.map((cat, idx) => (
              <div key={idx} className="space-y-1">
                <div className="flex items-center justify-between text-[10px] font-sans">
                  <span className="font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-widest text-[9px]">{cat.label}</span>
                  <span className="font-mono font-black text-slate-900 dark:text-white">{cat.value}%</span>
                </div>
        ...
        {evaluation.summary && (
          <div className="space-y-2">
            ...
            <p className="text-xs font-sans text-slate-655 dark:text-slate-350 leading-relaxed bg-brand-500/5 p-4 rounded-xl border border-brand-500/10 font-medium m-0">
              {evaluation.summary}
            </p>
          </div>
        )}
            ...
            <ul className="space-y-2 m-0 p-0 list-none">
              {evaluation.strengths.length > 0 ? (
                evaluation.strengths.map((str, i) => (
                  <li key={i} className="flex items-start gap-2.5 text-xs text-slate-655 dark:text-slate-350 font-sans leading-normal font-medium">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 pt-3 border-t border-slate-100 dark:border-slate-850/80">
            ...
            {evaluation.missing_topics.length > 0 && (
              <div className="space-y-2 text-left">
                <span className="block text-[9px] font-black uppercase tracking-widest text-slate-405 dark:text-slate-500">
