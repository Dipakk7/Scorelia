...
export function EmptyPerformanceState({
  onRetry,
  message = 'No platform performance metrics or cluster health telemetry recorded.',
  className = '',
  ...
  return (
    ...
    >
      <div className="p-4 rounded-2xl bg-purple-500/10 text-purple-400 border border-purple-500/20 shadow-lg shadow-purple-950/30">
        <Activity size={32} className="stroke-[1.75]" />
      </div>

      <div className="max-w-md space-y-1.5">
        <h3 className="text-base font-bold text-slate-100 m-0">
          Performance Telemetry Offline
        </h3>
      ...
      {onRetry && (
        ...
        >
          ...
          <span>Retry Cluster Telemetry</span>
        </button>
      )}
    </div>
  )
