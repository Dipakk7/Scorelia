...
export const CodeQualityOverview: React.FC<CodeQualityOverviewProps> = ({
  ...
  const safeMetrics = metrics ?? githubDeveloperMetricsMockData.codeQuality
  const overallScore = safeMetrics?.overallScore ?? (safeMetrics as any)?.codeQualityScore ?? 0
  const healthGrade = safeMetrics?.healthGrade ?? (safeMetrics as any)?.maintainabilityGrade ?? 'Good'
  const testCoverage = safeMetrics?.testCoverage ?? 0
  const technicalDebt = safeMetrics?.technicalDebt ?? (safeMetrics as any)?.technicalDebtHours ?? 0
  const maintainability = safeMetrics?.maintainability ?? 'A'
  const securityScore = safeMetrics?.securityScore ?? 0
  const reliability = safeMetrics?.reliability ?? 0
  const lintScore = safeMetrics?.lintScore ?? 0
  const documentationScore = safeMetrics?.documentationScore ?? 0
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <QualityScoreBadge score={`${overallScore}%`} level={healthGrade} />
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="p-3 rounded-xl border border-slate-700/80 bg-slate-900/80 text-center">
          ...
          <div className="text-xl font-extrabold text-emerald-400 mt-1 font-mono">{testCoverage}%</div>
        ...
        <div className="p-3 rounded-xl border border-slate-700/80 bg-slate-900/80 text-center">
          ...
          <div className="text-xl font-extrabold text-white mt-1 font-mono">{technicalDebt}</div>
        ...
        <div className="p-3 rounded-xl border border-slate-700/80 bg-slate-900/80 text-center">
          ...
          <div className="text-xl font-extrabold text-sky-400 mt-1 font-mono">{maintainability}</div>
        ...
        <div className="p-3 rounded-xl border border-slate-700/80 bg-slate-900/80 text-center">
          ...
          <div className="text-xl font-extrabold text-purple-400 mt-1 font-mono">{securityScore}%</div>
      ...
      <div className="space-y-2.5 py-1 text-xs">
        <div className="space-y-1">
          <div className="flex justify-between text-[11px] font-semibold">
            ...
            <span className="text-emerald-400 font-mono">{reliability}%</span>
          </div>
        ...
        <div className="space-y-1">
          <div className="flex justify-between text-[11px] font-semibold">
            ...
            <span className="text-sky-400 font-mono">{lintScore}%</span>
          </div>
          <div className="h-1.5 w-full rounded-full bg-slate-900 overflow-hidden border border-slate-800">
        ...
        <div className="space-y-1">
          <div className="flex justify-between text-[11px] font-semibold">
            ...
            <span className="text-purple-400 font-mono">{documentationScore}%</span>
          </div>
          <div className="h-1.5 w-full rounded-full bg-slate-900 overflow-hidden border border-slate-800">
