...
const TEMPLATE_OPTIONS: TemplateOption[] = [
  ...
  },
  {
    ...
    icon: Briefcase,
    tagColor: 'bg-brand-500/10 text-brand-500 border-brand-500/20',
  },
  {
    id: 'EXECUTIVE',
    name: 'Executive',
        ...
        {TEMPLATE_OPTIONS.map((template) => {
          ...
          return (
            <button
              ...
              className={cn(
                'group relative text-left p-5 rounded-2xl border transition-all duration-200 cursor-pointer focus:outline-none flex flex-col justify-between h-full bg-white dark:bg-dark-card hover:translate-y-[-2px]',
                isSelected
                  ? 'border-brand-500 ring-2 ring-brand-500/20 shadow-lg shadow-brand-500/5'
                  : 'border-slate-200/80 dark:border-slate-800 hover:border-slate-350 dark:hover:border-slate-700 shadow-xs',
                disabled && 'opacity-65 cursor-not-allowed hover:translate-y-0'
              )}
            >
              <div className="flex items-start gap-4 mb-4">
                <div
                  className={cn(
                    'p-3 rounded-xl border flex-shrink-0 transition-colors duration-200',
                    isSelected
                      ? 'bg-brand-600 border-brand-500 text-white'
                      : 'bg-slate-50 dark:bg-slate-900 border-slate-200/60 dark:border-slate-800 text-slate-500 dark:text-slate-400 group-hover:text-brand-500 dark:group-hover:text-brand-400'
                  )}
                >
                  <Icon size={20} />
                </div>
                <div className="space-y-1">
                  <h4 className="font-semibold text-slate-850 dark:text-slate-100 font-display text-sm">
                    {template.name}
                  </h4>
                  <span className={cn('inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium border uppercase tracking-wider', template.tagColor)}>
                    {template.id.replace('_', ' ')}
                  </span>
                </div>
              </div>
              <p className="text-xs text-slate-550 dark:text-slate-400 leading-relaxed font-sans mt-auto">
