    ...
    <div className="space-y-5 animate-fade-in text-left">
      ...
      {categories.length === 0 && (
        <div className="bg-[#121424]/95 border border-dashed border-slate-800/90 rounded-2xl p-8 text-center space-y-3">
          ...
          <div className="space-y-1">
            <h4 className="text-sm font-bold text-white m-0">No Skill Categories Added Yet</h4>
      ...
      <div className="space-y-3">
        {categories.map((cat, idx) => (
          <div
            key={cat.id}
            className="bg-[#121424]/95 border border-slate-800/90 rounded-2xl p-4 space-y-3 shadow-sm group"
          >
            <div className="flex items-center justify-between border-b border-slate-800/80 pb-2">
              <div className="flex items-center gap-2">
                <div className="flex items-center gap-0.5 text-slate-400 group-hover:text-slate-200">
                ...
                <input
                  ...
                  className="text-xs font-extrabold text-white bg-transparent border-b border-transparent hover:border-slate-700 focus:border-purple-500 focus:outline-none px-1 py-0.5 rounded transition-colors font-display"
                />
              </div>
              ...
              <button
                ...
                className="text-xs text-slate-400 hover:text-pink-400 cursor-pointer p-1 focus:outline-none"
            ...
            <div className="flex flex-wrap items-center gap-1.5 pt-1">
              {cat.skills.map((skill, sIdx) => (
                <span
                  key={sIdx}
                  className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-bold bg-purple-950/50 text-purple-200 border border-purple-500/35 group/tag font-mono"
                >
                  <span>{skill}</span>
                  <button
                    type="button"
                    onClick={() => handleRemoveSkillTag(cat.id, sIdx)}
                    className="text-purple-400 hover:text-white cursor-pointer opacity-70 group-hover/tag:opacity-100 focus:outline-none"
                  >
                    <X size={11} />
                  </button>
                </span>
              ...
              <div className="flex items-center gap-1">
                <input
                  ...
                  className="bg-[#0e101c] border border-slate-800/90 hover:border-slate-700/80 rounded-lg px-2.5 py-1 text-xs font-semibold text-white placeholder:text-slate-500 focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 w-36 transition-all"
                />
              </div>
            </div>
          </div>
