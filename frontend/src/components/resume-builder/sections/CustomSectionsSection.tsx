            ...
            >
              ...
              {isExpanded && (
                <div className="p-4 md:p-5 space-y-4 border-t border-slate-200/80 dark:border-white/[0.08]">
                  ...
                  <div className="space-y-3 pt-2 border-t border-slate-200/80 dark:border-white/[0.08]">
                    <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Section Entries</label>
                    {sec.items.map((item, itemIdx) => (
                      <div key={item.id} className="p-3 bg-white dark:bg-[#1f2238] border border-slate-200/80 dark:border-white/[0.1] rounded-xl space-y-2">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                          <input
                            type="text"
                            defaultValue={item.title}
                            placeholder="Entry Title / Role"
                            className="bg-slate-50/90 dark:bg-[#171a2b] border border-slate-200/80 dark:border-white/[0.08] rounded-lg px-3 py-1.5 text-xs font-bold text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:border-purple-500/80 focus-visible:ring-2 focus-visible:ring-purple-500/80 transition-all"
                          />
                          <input
                            type="text"
                            defaultValue={item.subtitle}
                            placeholder="Subtitle / Date / Organization"
                            className="bg-slate-50/90 dark:bg-[#171a2b] border border-slate-200/80 dark:border-white/[0.08] rounded-lg px-3 py-1.5 text-xs font-medium text-slate-700 dark:text-slate-300 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:border-purple-500/80 focus-visible:ring-2 focus-visible:ring-purple-500/80 transition-all"
                          />
                        </div>
                        <textarea
                          rows={2}
                          ...
                          className="w-full bg-slate-50/90 dark:bg-[#171a2b] border border-slate-200/80 dark:border-white/[0.08] rounded-lg p-2.5 text-xs font-medium text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:border-purple-500/80 focus-visible:ring-2 focus-visible:ring-purple-500/80 resize-none transition-all"
                        />
                      </div>
                    ))}
