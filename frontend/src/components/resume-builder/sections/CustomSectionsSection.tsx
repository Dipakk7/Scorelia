      ...
      <div className="space-y-4">
        {sections.map((sec, idx) => {
          ...
          return (
            <div
              key={sec.id}
              className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-white/10 rounded-xl overflow-hidden shadow-sm transition-colors"
            >
              {/* Header Bar */}
              <div
                onClick={() => toggleExpand(sec.id)}
                className="flex items-center justify-between p-4 bg-white/80 dark:bg-slate-950/40 cursor-pointer hover:bg-slate-100 dark:hover:bg-white/5 transition-colors border-b border-slate-200/80 dark:border-white/5"
              >
              ...
              {isExpanded && (
                <div className="p-4 md:p-5 space-y-4 border-t border-slate-200/80 dark:border-white/5">
                  ...
                  <div className="space-y-3 pt-2 border-t border-slate-200/80 dark:border-white/5">
                    <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Section Entries</label>
                    {sec.items.map((item, itemIdx) => (
                      <div key={item.id} className="p-3 bg-white dark:bg-slate-950/70 border border-slate-200 dark:border-white/10 rounded-xl space-y-2">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                          <input
                            type="text"
                            defaultValue={item.title}
                            placeholder="Entry Title / Role"
                            className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-lg px-3 py-1.5 text-xs font-bold text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:border-purple-500/80 focus-visible:ring-2 focus-visible:ring-purple-500/80 transition-colors"
                          />
                          <input
                            type="text"
                            defaultValue={item.subtitle}
                            placeholder="Subtitle / Date / Organization"
                            className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-lg px-3 py-1.5 text-xs font-medium text-slate-700 dark:text-slate-300 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:border-purple-500/80 focus-visible:ring-2 focus-visible:ring-purple-500/80 transition-colors"
                          />
                        </div>
                        <textarea
                          rows={2}
