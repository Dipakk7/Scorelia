    ...
    <div className={cn('flex flex-col gap-4 text-left font-sans text-xs', className)}>
      ...
      <div className="flex items-center justify-between select-none">
        <div className="flex items-center gap-2 text-slate-700 dark:text-slate-350">
          ...
          <span className="text-xs font-black uppercase tracking-wider text-foreground leading-none">
            Execution Steps Timeline ({steps.length})
          </span>
        </div>

        {totalDurationMs !== undefined && (
          <div className="flex items-center gap-1.5 text-[10px] font-mono text-muted-foreground font-bold leading-none">
      ...
      <div className="relative flex flex-col gap-5 pl-4 border-l-2 border-slate-200 dark:border-slate-850 ml-2 mt-1">
        {steps.map((step, idx) => (
          <div key={idx} className="relative">
            ...
            <div
              className={cn(
                'absolute -left-[23px] top-4.5 h-3.5 w-3.5 rounded-full border-2 bg-card z-10 transition-colors duration-250',
                step.status === 'success'
                  ? 'border-emerald-500 bg-card'
                  : 'border-rose-500 bg-card'
              )}
            />
        ...
        {steps.length === 0 && (
          <div className="flex flex-col items-center justify-center p-8 border border-dashed border-slate-205 dark:border-slate-850 rounded-2xl text-muted-foreground bg-card/70 backdrop-blur-md">
