      ...
      <div className="p-5 rounded-2xl bg-[var(--surface)] border border-[var(--border)] shadow-[var(--shadow-sm)] space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-sm font-bold text-[var(--heading)] tracking-tight">Ingestion Pipeline</h3>
              <p className="text-[11px] font-medium text-[var(--muted)] font-mono">Parser: Unstructured + PyPDF</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-2 text-xs">
          <div className="p-2.5 rounded-xl bg-[var(--surface-hover)] border border-[var(--border)]">
            <div className="text-[10px] text-[var(--muted)]">Average Chunk Size</div>
            <div className="font-bold text-[var(--heading)] text-xs mt-0.5 font-mono">512 Tokens</div>
          </div>
          <div className="p-2.5 rounded-xl bg-[var(--surface-hover)] border border-[var(--border)]">
            <div className="text-[10px] text-[var(--muted)]">Chunk Overlap</div>
            <div className="font-bold text-[var(--heading)] text-xs mt-0.5 font-mono">64 Tokens</div>
          </div>
        </div>

        {/* Supported Format Distribution */}
        <div className="space-y-2 pt-1">
          <div className="text-[11px] font-bold text-[var(--heading)] uppercase tracking-wider">Format Breakdown</div>
          {fileTypeDistribution.map((item, idx) => {
            const Icon = item.icon
            return (
              <div key={idx} className="flex items-center justify-between p-2 rounded-xl bg-[var(--surface-hover)] border border-[var(--border)] text-xs">
                <div className="flex items-center gap-2">
                  <div className={cn('p-1 rounded-lg border', item.color)}>
                    <Icon size={13} />
                  </div>
                  <span className="font-semibold text-[var(--heading)]">{item.format}</span>
                </div>
                <div className="flex items-center gap-2 font-mono text-[11px]">
                  <span className="text-[var(--heading)] font-bold">{item.count}</span>
                  <span className="text-[var(--muted)]">({item.percentage})</span>
                </div>
              </div>
            )
          })}
