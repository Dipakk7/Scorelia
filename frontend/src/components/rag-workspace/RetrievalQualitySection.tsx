...
export function RetrievalQualitySection({ className }: RetrievalQualitySectionProps) {
  return (
    <div className={cn('grid grid-cols-1 lg:grid-cols-12 gap-6', className)}>
      ...
      <div className="lg:col-span-7 p-5 rounded-2xl bg-[#0e0f1a]/90 border border-white/10 shadow-lg text-left space-y-4">
        ...
        <div className="h-56 w-full pt-1">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={MOCK_SIMILARITY_DISTRIBUTION} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <Tooltip
                contentStyle={{ backgroundColor: '#121320', borderColor: 'rgba(255,255,255,0.1)', borderRadius: '12px', fontSize: '11px', color: '#e2e8f0' }}
              />
        ...
        <div className="p-5 rounded-2xl bg-[#0e0f1a]/90 border border-white/10 shadow-lg text-left space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3 text-xs">
            <div className="p-3 rounded-xl bg-[#121320] border border-white/5 space-y-1">
              ...
              <span className="text-lg font-extrabold text-emerald-400 font-mono">94.8%</span>
            ...
            <div className="p-3 rounded-xl bg-[#121320] border border-white/5 space-y-1">
              ...
              <span className="text-lg font-extrabold text-purple-400 font-mono">97.3%</span>
          ...
          <div className="space-y-1.5 pt-1 text-xs">
            {MOCK_CONFIDENCE_BREAKDOWN.map((item, idx) => (
              <div key={idx} className="flex items-center justify-between text-[11px]">
                ...
                <span className="font-mono text-slate-200 font-bold">{item.percentage}% ({item.count})</span>
              </div>
            ))}
          </div>
        </div>
