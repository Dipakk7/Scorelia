  ...
  return (
    <div
      className={cn(
        ...
        fullScreen && 'fixed inset-0 z-50 bg-[var(--background)]/85 backdrop-blur-sm min-h-screen',
        className
      )}
    >
      <Spinner size={fullScreen ? 'lg' : 'md'} />
      {label && (
        <span className="text-sm font-medium text-[var(--muted)] font-display animate-pulse">
          {label}
        </span>
      )}
    </div>
