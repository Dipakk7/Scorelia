    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3.5 items-stretch">
        {Array.from({ length: 6 }).map((_, i) => (
          <Card key={i} className="bg-[#10121e]/90 border border-white/10 rounded-2xl p-3.5 flex flex-col justify-between">
      ...
      {metrics.map((metric, index) => (
        ...
        >
          {metric.id === 'readiness' ? (
            ...
            <Card className="bg-[#10121e]/90 border border-white/10 rounded-2xl p-3.5 sm:p-4 flex flex-col justify-between hover:border-purple-500/30 hover:scale-[1.01] transition-all duration-200 h-full text-left overflow-hidden">
              <CardContent className="p-0 flex flex-col justify-between h-full space-y-2.5">
                {/* 1. Title (Top Left) */}
                <div className="flex items-center justify-start min-h-[18px]">
                ...
                <div className="flex flex-col items-center justify-center my-auto space-y-1 py-0.5">
                  ...
                  <div className="flex items-center justify-center">
                    <svg className="w-32 h-18 max-w-full" viewBox="0 0 120 72">
                      ...
                      />
                      {/* Score Value */}
                      <text x="60" y="38" textAnchor="middle" className="text-xl font-black font-mono fill-white">
                  ...
                  <Badge className="bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 text-[10px] font-bold px-2.5 py-0.5 rounded-md h-5 mt-2.5">
                    {metric.readinessTag || 'Excellent'}
                  </Badge>
                </div>

                {/* 3. Divider & Footer Row */}
                <div className="pt-2 border-t border-white/10 flex items-center justify-between text-[11px] font-medium text-slate-400 h-6 shrink-0 w-full">
            ...
            /* Perfectly Density-Aligned Metric Cards */
            <Card className="bg-[#10121e]/90 border border-white/10 rounded-2xl p-3.5 sm:p-4 flex flex-col justify-between hover:border-purple-500/30 hover:scale-[1.01] transition-all duration-200 h-full text-left overflow-hidden">
              <CardContent className="p-0 flex flex-col justify-between h-full space-y-2.5">
                {/* Header: Title */}
                ...
                <div className="space-y-1 my-0.5">
                  <div className="flex items-baseline gap-1.5 min-h-[28px] flex-wrap">
                    {typeof metric.value === 'number' ? (
                      <span className="text-2xl font-black text-white font-mono flex items-center gap-1.5 leading-none">
                        {metric.id === 'streak' && <Flame className="h-4.5 w-4.5 text-amber-500 fill-amber-500 shrink-0" />}
                ...
                <div className="pt-2 border-t border-white/10 flex items-center justify-between text-[11px] font-medium text-slate-400 h-6 shrink-0">
                  {metric.trend ? (
                    ...
                    <Badge
                      className={
                        metric.badgeVariant === 'success'
                          ? 'bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 text-[10px] font-bold px-2 py-0.5 rounded-md h-4.5 shrink-0'
                          : metric.badgeVariant === 'warning'
                          ? 'bg-amber-500/15 text-amber-400 border border-amber-500/30 text-[10px] font-bold px-2 py-0.5 rounded-md h-4.5 shrink-0'
                          : 'bg-purple-500/15 text-purple-300 border border-purple-500/30 text-[10px] font-bold px-2 py-0.5 rounded-md h-4.5 shrink-0'
                      }
                    >
                      {metric.badge}
                    </Badge>
