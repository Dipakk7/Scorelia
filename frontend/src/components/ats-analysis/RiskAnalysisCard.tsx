...
export const RiskAnalysisCard: React.FC = () => {
  ...
  return (
    <div className="rounded-2xl bg-slate-900/90 border border-slate-800/90 p-3.5 sm:p-4 shadow-lg space-y-3 h-full flex flex-col justify-between">
      {/* Header */}
        ...
        {mockRiskAnalysis.map((item) => {
          ...
          return (
            <div
              key={item.id}
              className={cn(
                'p-4 rounded-xl bg-slate-950/60 border transition-all duration-200 space-y-2.5 flex flex-col justify-between h-full',
                isCritical
