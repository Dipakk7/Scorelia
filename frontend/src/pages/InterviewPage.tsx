        ...
        <div className="space-y-6">
          ...
          {analyticsData && (
            <div className="space-y-3.5">
              <h3 className="font-display font-black text-sm text-slate-900 dark:text-white m-0">Performance Metrics Analysis</h3>
          ...
          <div className="space-y-3.5">
            <h3 className="font-display font-black text-sm text-slate-900 dark:text-white m-0">Simulated Mock Round History</h3>
            {sessions.length === 0 ? (
          ...
          <div className="lg:col-span-4 flex flex-col space-y-4">
            ...
            <Card className="border border-slate-205 dark:border-slate-855 bg-white/70 dark:bg-slate-900/40 backdrop-blur-md rounded-2xl shadow-sm hover:border-slate-350 dark:hover:border-slate-750 transition-all duration-300 text-left">
              <CardContent className="p-5 text-xs text-slate-655 dark:text-slate-400 font-sans space-y-3 leading-relaxed font-medium">
                <span className="text-[9px] font-black uppercase tracking-widest text-slate-405 dark:text-slate-500 block mb-1 leading-none">
          ...
          {reportLoading ? (
            ...
            <div className="space-y-6 text-left">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch">
                <Card className="md:col-span-2 border border-brand-500/10 bg-brand-500/5 dark:bg-brand-500/10 backdrop-blur-md rounded-2xl shadow-sm hover:border-slate-350 dark:hover:border-slate-750 transition-all duration-300 flex flex-col justify-between text-left">
                  <CardContent className="p-6 flex flex-col justify-between h-full space-y-4">
                    <div className="space-y-1.5 text-left">
                      ...
                      <h2 className="text-lg font-extrabold text-slate-900 dark:text-white line-clamp-1 m-0 leading-tight">
                    ...
                    <div className="flex gap-2">
                      <Button
                        ...
                        className="flex items-center justify-center gap-1.5 px-4 py-2.5 font-bold cursor-pointer rounded-xl border-slate-200 dark:border-slate-800 hover:border-brand-500/30 hover:bg-brand-500/5 transition-all bg-transparent text-xs h-9.5"
                      >
                        Return to Dashboard
                      </Button>
                    </div>
                ...
                {/* Score Dial Card */}
                <Card className="border border-slate-205 dark:border-slate-855 bg-white/70 dark:bg-slate-900/40 backdrop-blur-md rounded-2xl shadow-sm hover:border-slate-350 dark:hover:border-slate-750 transition-all duration-300 flex flex-col justify-center items-center text-center p-6">
                  <CardContent className="p-0 text-center space-y-3.5 flex flex-col items-center">
                    ...
                    <div className="space-y-1">
                      <h4 className="font-extrabold text-slate-905 dark:text-white text-xs m-0">Averaged Competency Rate</h4>
                      <p className="text-[10px] text-slate-450 dark:text-slate-500 font-sans font-bold m-0 leading-none">Across all completed questions</p>
                    </div>
                  </CardContent>
                </Card>
              </div>
                    ...
                    {[
                      ...
                      <div key={i} className="space-y-1">
                        <div className="flex items-center justify-between text-[10px] font-sans">
                          <span className="font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-widest text-[9px]">{m.label}</span>
                          <span className="font-mono font-black text-slate-900 dark:text-white">{m.value}%</span>
                        </div>
                ...
                <Card className="border border-slate-205 dark:border-slate-855 bg-white/70 dark:bg-slate-900/40 backdrop-blur-md rounded-2xl shadow-sm hover:border-slate-350 dark:hover:border-slate-750 transition-all duration-300 text-left">
                  ...
                  <CardContent className="p-5 space-y-2.5 overflow-y-auto max-h-[220px] text-left">
                    {reportData.weaknesses.map((weak, i) => (
                      <div key={i} className="flex items-start gap-2 text-[10px] text-slate-655 dark:text-slate-400 font-sans leading-relaxed font-medium">
                        <AlertTriangle size={12} className="text-rose-505 shrink-0 mt-0.5" />
                        <span>{weak}</span>
                      </div>
                    ))}
                  </CardContent>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
                ...
                {/* Recommendations */}
                <Card className="lg:col-span-4 border border-slate-205 dark:border-slate-855 bg-white/70 dark:bg-slate-900/40 backdrop-blur-md rounded-2xl shadow-sm hover:border-slate-350 dark:hover:border-slate-750 transition-all duration-300 text-left">
                  <CardHeader className="pb-2.5 border-b border-slate-100 dark:border-slate-800/60">
                    <CardTitle className="text-xs font-black uppercase tracking-wider text-slate-900 dark:text-white m-0">
                      Study & Coaching Plan
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="p-5 space-y-4 text-[10px] overflow-y-auto max-h-[300px] text-left">
                    {reportData.recommendations.practice_topics.length > 0 && (
                      <div className="space-y-1.5 text-left">
                        <strong className="text-slate-455 dark:text-slate-500 block uppercase tracking-widest text-[9px] font-black leading-none">Practice Topics</strong>
                    ...
                    {reportData.recommendations.learning_recommendations.length > 0 && (
                      <div className="space-y-1.5 text-left border-t border-slate-100 dark:border-slate-850 pt-3 mt-1">
                        <strong className="text-slate-455 dark:text-slate-500 block uppercase tracking-widest text-[9px] font-black leading-none">Coaching Advices</strong>
              ...
              {reportSessionData && (
                <div className="space-y-3.5">
                  <h3 className="font-display font-black text-sm text-slate-900 dark:text-white m-0">Review Detailed Response Logs</h3>
