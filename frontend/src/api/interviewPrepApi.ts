...
export class InterviewPrepApiService {
    ...
    try {
      ...
      return {
        metrics: [
          ...
          { id: 'completed_mocks', title: 'Mock Interviews', value: 12, unit: 'Completed', trend: '↑ 3 this week', trendType: 'positive', sparklinePoints: [10, 15, 12, 18, 22, 28, 30] },
          { id: 'avg_score', title: 'Average Score', value: '84%', unit: 'Across all mocks', trend: '↑ 6% vs last week', trendType: 'positive', sparklinePoints: [60, 65, 72, 75, 78, 82, 84] },
          { id: 'strong_areas', title: 'Strong Areas', value: 6, unit: 'Skills identified', badge: 'Improving', badgeVariant: 'success' },
          { id: 'areas_to_improve', title: 'Areas to Improve', value: 3, unit: 'Focus areas', badge: 'Needs attention', badgeVariant: 'warning' },
          { id: 'streak', title: 'Interview Streak', value: 5, unit: 'Days in a row', badge: 'Keep it up! 🔥', badgeVariant: 'orange' },
        ],
  ...
  /* 2. Mock Interviews APIs */
  static async getResumes(): Promise<ResumeOption[]> {
    try {
      const response = await api.get('/resumes')
      if (Array.isArray(response.data?.resumes)) {
        return response.data.resumes.map((r: any) => ({
          id: r.id,
          fileName: r.file_name || r.title || 'Resume.pdf',
          roleTarget: r.target_role || 'AI/ML Engineer',
      ...
      return [
        { id: 'res-1', fileName: 'AI_ML_Engineer_Resume_2026.pdf', roleTarget: 'AI/ML Engineer', lastUpdated: 'Updated 2 days ago', isDefault: true },
        { id: 'res-2', fileName: 'Senior_FullStack_Engineer.pdf', roleTarget: 'Full Stack Engineer', lastUpdated: 'Updated 1 week ago' },
        { id: 'res-3', fileName: 'Data_Scientist_Resume_V2.pdf', roleTarget: 'Data Scientist', lastUpdated: 'Updated 3 weeks ago' },
      ]
    }
  }

  static async getDifficulties(): Promise<DifficultyOption[]> {
    return [
      { id: 'easy', label: 'Easy', description: 'Fundamental concept checks and standard questions.', badgeVariant: 'emerald' },
      { id: 'medium', label: 'Medium', description: 'Real-world problem solving and core technical depth.', badgeVariant: 'amber' },
      { id: 'hard', label: 'Hard', description: 'Complex architecture, edge cases, and high-pressure scenarios.', badgeVariant: 'rose' },
      { id: 'adaptive', label: 'Adaptive AI', description: 'Dynamically scales difficulty based on your real-time responses.', badgeVariant: 'purple' },
    ]
  }

  static async getInterviewTypes(): Promise<InterviewTypeOption[]> {
    return [
      { id: 'technical', label: 'Technical', description: 'Coding algorithms, system architecture, framework internals.', iconName: 'Code2' },
      { id: 'hr', label: 'HR Round', description: 'Cultural fit, career aspirations, workplace scenarios.', iconName: 'UserCheck' },
      { id: 'behavioral', label: 'Behavioral', description: 'STAR method storytelling, leadership principles, team conflict.', iconName: 'MessageSquare' },
      { id: 'mixed', label: 'Mixed Comprehensive', description: 'Combination of technical questions and behavioral scenarios.', iconName: 'Layers' },
    ]
  }

  static async getInterviewModes(): Promise<InterviewModeOption[]> {
    return [
      { id: 'voice', label: 'Voice Interactive', description: 'Spoken real-time AI conversation with voice feedback.', iconName: 'Mic' },
      { id: 'text', label: 'Text Prompt', description: 'Written Q&A interface for quick practice sessions.', iconName: 'FileText' },
      { id: 'mixed', label: 'Voice + Text Hybrid', description: 'Speak or type your responses as you prefer.', iconName: 'Sparkles' },
    ]
  }
  ...
  static async getRecentMockHistory(): Promise<MockInterviewHistoryItem[]> {
    try {
      ...
      return [
        { id: 'mock-hist-1', date: 'May 20, 2026', company: 'Google', role: 'AI/ML Engineer', interviewType: 'Technical', durationMinutes: 45, scorePercent: 88, status: 'Completed' },
        { id: 'mock-hist-2', date: 'May 16, 2026', company: 'Meta', role: 'Senior Machine Learning Engineer', interviewType: 'System Design', durationMinutes: 60, scorePercent: 82, status: 'Completed' },
        { id: 'mock-hist-3', date: 'May 10, 2026', company: 'Amazon', role: 'Software Development Engineer', interviewType: 'Behavioral (STAR)', durationMinutes: 30, scorePercent: 79, status: 'Completed' },
        { id: 'mock-hist-4', date: 'May 04, 2026', company: 'Microsoft', role: 'Full Stack Engineer', interviewType: 'Mixed Technical', durationMinutes: 45, scorePercent: 91, status: 'Completed' },
      ]
    }
  }
  ...
  static async getQuestionCategories(): Promise<QuestionCategory[]> {
    try {
      const response = await api.get('/interview-prep/question-categories')
      return response.data
    } catch {
      return [
  ...
  static async getQuestionBankList(): Promise<QuestionBankItem[]> {
    try {
      const response = await api.get('/interview-prep/questions')
      return response.data
    } catch {
      return [
        {
  ...
  static async getPracticeSummary(): Promise<PracticeSummaryData> {
    return {
      totalAvailable: 1250,
      totalCompleted: 524,
      totalBookmarked: 42,
  ...
  static async getRecentlyPracticedList(): Promise<RecentlyPracticedQuestionItem[]> {
    return [
      { id: 'rp-1', questionTitle: 'Explain the Bias-Variance Tradeoff in Machine Learning models.', categoryLabel: 'Machine Learning', completionDate: 'Today, 2:30 PM', practiceScorePercent: 92 },
      { id: 'rp-2', questionTitle: 'Optimize a slow SQL query with multiple JOINs and GROUP BY aggregation.', categoryLabel: 'SQL & Databases', completionDate: 'Yesterday', practiceScorePercent: 88 },
      { id: 'rp-3', questionTitle: 'Tell me about a time you resolved a major technical conflict on your team.', categoryLabel: 'Behavioral & STAR', completionDate: '3 days ago', practiceScorePercent: 85 },
    ]
  }

  /* 4. My Answers APIs */
  static async getAnswerSummary(): Promise<AnswerSummaryData> {
    return {
      totalAnswers: 38,
      avgScorePercent: 86,
  ...
  static async getAnswerHistoryList(): Promise<AnswerHistoryItem[]> {
    try {
      const response = await api.get('/interview-prep/answers')
      return response.data
    } catch {
      return [
        { id: 'ans-1', questionTitle: 'Explain the Bias-Variance Tradeoff in Machine Learning models.', categoryLabel: 'Machine Learning', companyName: 'Google', attemptDate: 'May 20, 2026', scorePercent: 92, durationText: '4 min 12 sec', attemptNumber: 2, source: 'Mock Interview', difficulty: 'Medium', resultTag: 'Excellent', isBookmarked: true, isFavorite: true },
        { id: 'ans-2', questionTitle: 'Design a scalable Distributed Vector Database for AI embeddings.', categoryLabel: 'System Design', companyName: 'OpenAI', attemptDate: 'May 18, 2026', scorePercent: 84, durationText: '11 min 45 sec', attemptNumber: 1, source: 'Mock Interview', difficulty: 'Hard', resultTag: 'Good', isBookmarked: false, isFavorite: false },
        { id: 'ans-3', questionTitle: 'How do Transformer Self-Attention mechanisms calculate Q, K, V matrices?', categoryLabel: 'Deep Learning', companyName: 'Meta', attemptDate: 'May 15, 2026', scorePercent: 78, durationText: '6 min 30 sec', attemptNumber: 1, source: 'Question Bank', difficulty: 'Hard', resultTag: 'Good', isBookmarked: true, isFavorite: false },
        { id: 'ans-4', questionTitle: 'Tell me about a time you resolved a major technical conflict on your team.', categoryLabel: 'Behavioral & STAR', companyName: 'Microsoft', attemptDate: 'May 12, 2026', scorePercent: 95, durationText: '3 min 50 sec', attemptNumber: 3, source: 'Mock Interview', difficulty: 'Easy', resultTag: 'Excellent', isBookmarked: false, isFavorite: true },
        { id: 'ans-5', questionTitle: 'Optimize a slow SQL query with multiple JOINs and GROUP BY aggregation.', categoryLabel: 'SQL & Databases', companyName: 'Amazon', attemptDate: 'May 08, 2026', scorePercent: 68, durationText: '5 min 15 sec', attemptNumber: 1, source: 'Question Bank', difficulty: 'Medium', resultTag: 'Needs Improvement', isBookmarked: false, isFavorite: false },
      ]
    }
  }

  static async getAnswerDetails(id?: string): Promise<AnswerDetailData> {
    try {
      const response = await api.get(`/interview-prep/answers/${id || 'ans-1'}`)
      return response.data
    } catch {
      return {
        id: id || 'ans-1',
        questionTitle: 'Explain the Bias-Variance Tradeoff in Machine Learning models.',
        userAnswerText: 'The Bias-Variance tradeoff represents the tension between a model being too simplistic (high bias, leading to underfitting) versus being overly complex (high variance, leading to overfitting). High bias occurs when the model fails to capture underlying patterns, resulting in systematic errors on both training and validation sets. High variance occurs when the model learns noise in the training data, performing exceptionally well on training but poorly on unseen test data. To optimize performance, we balance model capacity using regularization techniques like L1/L2 penalties, dropout in neural nets, or ensemble methods like Bagging (which reduces variance) and Boosting (which reduces bias).',
        expectedAnswerText: 'A comprehensive answer should: 1) Mathematically decompose expected test error into Bias^2 + Variance + Irreducible Error. 2) Explain Bias as error from wrong model assumptions (underfitting) and Variance as sensitivity to small fluctuations in the training set (overfitting). 3) Contrast algorithms: Linear Regression/Naive Bayes have high bias, whereas deep decision trees and unregularized neural nets have high variance. 4) Detail mitigation techniques: Cross-validation, Regularization (L1/L2), Early Stopping, and Ensemble Methods (Random Forest reduces variance via bootstrap aggregating; Gradient Boosting iteratively reduces bias).',
        keySkillsTested: ['ML Theory & Generalization', 'Overfitting & Underfitting Diagnosis', 'Regularization & Ensembles'],
        difficulty: 'Medium',
        ...
        feedback: {
          ...
          strengths: ['Clear definition of bias (underfitting) vs variance (overfitting).', 'Accurately highlighted ensemble methods (Bagging for variance, Boosting for bias).', 'Articulate delivery with structured terminology.'],
          areasToImprove: ['Explicitly mention the mathematical decomposition: Expected Error = Bias^2 + Variance + Irreducible Noise.', 'Provide concrete code example or library reference (e.g. scikit-learn Ridge/Lasso hyperparameters).'],
  ...
  static async getImprovementSuggestions(): Promise<ImprovementSuggestionData> {
    return {
      recommendedPracticeTopic: 'System Design & Scalability',
      suggestedTopics: ['Distributed Caching & Redis', 'Microservices API Gateways', 'SQL Indexing & Query Tuning'],
  ...
  static async getRecentAttemptsList(): Promise<RecentAttemptItem[]> {
    return [
      { id: 'ra-1', questionTitle: 'Explain the Bias-Variance Tradeoff in Machine Learning models.', scorePercent: 92, attemptDate: 'May 20, 2026', improvementPercent: 14 },
      { id: 'ra-2', questionTitle: 'Design a scalable Distributed Vector Database for AI embeddings.', scorePercent: 84, attemptDate: 'May 18, 2026', improvementPercent: 8 },
      { id: 'ra-3', questionTitle: 'Tell me about a time you resolved a major technical conflict on your team.', scorePercent: 95, attemptDate: 'May 12, 2026', improvementPercent: 18 },
    ]
  }

  /* 5. Performance Analytics APIs */
  static async getPerformanceAnalyticsData(): Promise<PerformanceWorkspaceData> {
    try {
      const response = await api.get('/interview-prep/performance')
      return response.data
    } catch {
      return {
        summaryCards: [
          { id: 'overall_score', title: 'Overall Readiness Score', value: 87, unit: '/100', trendText: '↑ 5% vs last month', trendType: 'positive', iconName: 'Award', badgeText: 'Top 18%' },
          { id: 'interviews_completed', title: 'Interviews Completed', value: 12, unit: 'Rounds', trendText: '↑ 3 this week', trendType: 'positive', iconName: 'Video' },
          { id: 'practice_sessions', title: 'Practice Sessions', value: 38, unit: 'Questions', trendText: '↑ 8 this week', trendType: 'positive', iconName: 'BookOpen' },
          { id: 'avg_accuracy', title: 'Average Accuracy', value: '84%', unit: 'Across all rounds', trendText: '↑ 6% vs baseline', trendType: 'positive', iconName: 'Target' },
          { id: 'improvement_gain', title: 'Improvement Gain', value: '+14%', unit: 'Since start', trendText: 'Consistent growth', trendType: 'positive', iconName: 'TrendingUp' },
          { id: 'total_practice_time', title: 'Total Practice Time', value: '28.5', unit: 'Hours', trendText: 'Avg 4.5 hrs/week', trendType: 'neutral', iconName: 'Clock' },
        ],
        ...
        timelineEvents: [
          { id: 'tl-1', date: 'May 20, 2026', type: 'Mock Interview', title: 'Completed Google AI/ML Mock Round #2', description: 'Achieved overall 88% score with strong STAR structure feedback.', badgeText: '88% Score' },
          { id: 'tl-2', date: 'May 16, 2026', type: 'Question Bank Milestone', title: 'Passed 500+ Question Bank Milestones', description: 'Completed 500th technical problem across Python, SQL, and System Design.', badgeText: '500+ Questions' },
          { id: 'tl-3', date: 'May 12, 2026', type: 'Streak Milestone', title: 'Hit 5-Day Active Practice Streak', description: 'Logged consecutive practice sessions for 5 days in a row.', badgeText: '🔥 5 Days Streak' },
          { id: 'tl-4', date: 'May 04, 2026', type: 'Score Achievement', title: 'Breakthrough 90%+ in Behavioral Round', description: 'Achieved highest STAR score rating (95%) in Leadership round.', badgeText: '95% Peak Score' },
        ],
        goalTracker: {
        ...
        performanceSidebar: {
          ...
          weeklyHighlights: ['Increased System Design score by +12%', 'Maintained a 5-day practice streak', 'Completed 3 full-length mock interviews'],
          bestPerformanceTitle: 'Behavioral Leadership Round',
          bestPerformanceScore: 95,
          areasRequiringAttention: ['Distributed System Design (72%)', 'Deep Learning Transformer Math (61%)'],
          achievementBadges: [
  ...
  /* 6. Copilot APIs */
  static async getCopilotWorkspaceData(): Promise<InterviewCopilotWorkspaceData> {
    try {
      const response = await api.get('/interview-prep/copilot')
      return response.data
    } catch {
      return {
        messages: [
...
print("Test Score:", model.score(X_test, y_test))`,
  ...
  static async sendCopilotPrompt(prompt: string): Promise<CopilotChatMessage> {
    try {
      const response = await api.post('/interview-prep/copilot/message', { prompt })
      return response.data
    } catch {
      return {
        id: `ai-msg-${Date.now()}`,
        sender: 'assistant',
        text: `Here is my Copilot analysis for: "${prompt}"`,
        bulletPoints: [
          'Evaluated against your active Google AI/ML target role context.',
          'Key recommendation: Maintain structured STAR storytelling format with quantified business results.',
          'Ensure O(N) or O(N log N) algorithmic optimization for technical drills.',
        ],
  ...
  /* 7. Reports & Export APIs */
  static async getReportsWorkspaceData(): Promise<ReportsWorkspaceData> {
    try {
      const response = await api.get('/interview-prep/reports')
      return response.data
    } catch {
      return {
        summary: {
          overallReadinessScore: 87,
          overallScorePercent: 86,
          interviewCount: 12,
          questionCount: 524,
          avgAccuracyPercent: 84,
          strongestSkill: 'STAR Behavioral Storytelling (94%)',
          weakestSkill: 'ANN Vector Database Indexing (61%)',
          totalPracticeTimeHours: 28.5,
        },
        availableReports: [
          {
            reportId: 'rep-1',
            ...
            title: 'FAANG Interview Readiness Diagnostic Report',
            summaryText: 'Comprehensive evaluation of candidate readiness for Senior AI/ML Engineer roles at Google, Meta, and Amazon.',
            keyHighlights: [
              'Readiness Score: 87/100 (Top 18% candidate pool)',
              '12 Mock Rounds & 524 Practice Problems completed',
              'Peak performance in Behavioral Leadership (95%) & ML Theory (92%)',
            ],
          },
          {
            reportId: 'rep-2',
            generatedDate: 'May 16, 2026',
            reportType: 'Skill Assessment',
            title: 'Technical & Behavioral Competency Matrix',
            summaryText: 'Detailed breakdown of engineering skills across Python, SQL, Machine Learning, Deep Learning, and Communication.',
            keyHighlights: [
              'Python Standard Library & PyTorch: Expert (88%)',
              'SQL Database Query Tuning: Intermediate (76%)',
              'System Design Sharding & Vector Indexing: Focus Needed (72%)',
            ],
          },
          {
            reportId: 'rep-3',
            ...
            reportType: 'Mock',
            title: 'Google AI/ML Mock Interview Performance Audit',
            summaryText: 'Round evaluation for 60-minute technical interview covering system architecture and machine learning algorithms.',
            keyHighlights: [
              'Overall Mock Score: 88% (Passed)',
              'STAR Methodology Score: 94% (Structured response)',
              'Pacing & Time Allocation: 45 Minutes',
            ],
          },
        ],
        exportFormats: [
          { format: 'PDF', label: 'PDF Document', isAvailable: true },
...
export default InterviewPrepApiService
